...
These dates define the **overall lifespan** (outer bounds) of the maintenance rule. The specific execution times are defined in the **Periods** tab.
...
> **Best Practice:** Use a common time zone (e.g., **UTC**) across the Zabbix Server, the database, and the PHP frontend configuration for predictable results, especially with recurring periods and DST changes.

---

## ⚙️ Backend Process and Configuration

### The Timer Process: The Engine of Maintenance

The process responsible for calculating and initiating maintenance periods is the **Zabbix Timer process** (controlled by `StartTimers`).

1.  **Recalculation Interval (The Check):** The Timer process checks whether a host's status must change to/from maintenance at **0 seconds of every minute**.
    * This means the maintenance window will start and stop with a **maximum latency of 60 seconds** after the scheduled time.
2.  **Configuration Cache:** Maintenance definitions are stored in the database and loaded into the Zabbix Server's **Configuration Cache**. The Timer uses this cache to determine host status.
...
3.  Any problem event is filtered by the Action: If the host is in maintenance AND the problem has the tag `service:database`, the alert is **suppressed**.
4.  If a hardware problem (e.g., no matching tag) occurs, the alert **will still be sent**, as only the database service issues are suppressed.
...
The key method used for maintenance management is **`maintenance.create`** (and corresponding `.get`, `.update`, and `.delete` methods).
...
When calling the `maintenance.create` method, you send a JSON-RPC request containing several crucial parameters that mirror the frontend configuration:
...
1.  **`timeperiod_type`**: The type of schedule (`0` for one-time, `4` for monthly, etc.).
2.  **`start_time`**: The time of day the maintenance starts (in seconds from midnight, **based on Zabbix Server Time Zone**).
3.  **`period`**: The duration of the maintenance window in seconds.
4.  **`dayofweek` / `dayofmonth`**: Used for recurring schedules.

-----

## 🚀 Practical Example: Creating Maintenance via `curl`

This `curl` command demonstrates how to programmatically create the "Last Sunday of the Month" maintenance window, which runs at midnight for 2 hours, and applies to host group ID `42`.

### Prerequisites

You must replace `<ZABBIX_SERVER_IP_OR_DNS>` with your server's address and `"your_auth_token"` with an active token from a successful `user.login` API call. The **Host Group ID** (e.g., `"42"`) must be known beforehand.
