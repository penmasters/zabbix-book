...
- you need to skip the database population step on all but the first Zabbix server as
  the database is shared between all Zabbix servers.
...
- you make sure that all Zabbix servers can connect to the database server.
  For example, if you are using PostgreSQL, ensure that the `pg_hba.conf` file 
...
These log messages confirm that the HA manager process has started and has assumed
...
???+ note
    There are several options available for clustering software and load-balancers,
...
Keepalived is a minimal type of clustering software that enables the use of 
a (VIP) for frontend services, ensuring seamless failover and service continuity.
...
Once the frontend is installed on all servers, we need to start and enable
