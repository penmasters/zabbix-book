...
/// figure-caption | 4.76 #figure-4-76
Zabbix IPMI monitoring architecture
///
...
???+ example "Example trigger expression"
...
The number of IPMI pollers is controlled by the Zabbix server or proxy configuration
parameter `StartIPMIPollers`.
...
Because IPMI pollers rely on network communication with management controllers,
their performance is influenced not only by Zabbix itself, but also by:
...
Although Zabbix does not use `ipmitool` internally, the two tools are closely
related conceptually. Both rely on standard IPMI mechanisms and expose the same
...
If a sensor cannot be queried successfully with ipmitool, it will almost 
certainly fail in Zabbix as well.

---

#### Configuring an IPMI Item in Zabbix

Configuring an IPMI item in Zabbix is conceptually simple, but attention to 
detail is crucial.
...
???+ example "IPMI item key for CPU temperature"
...
In the following section, we will show how to run the simulator using Podman 
resulting in a Redfish service listening on port 8000. You can also run the
simulator using Docker, by replacing `podman` with `docker` in the commands below.
...
**Clone and build the Redfish Mockup Server**

Clone the Redfish Mockup Server Repository.

!!! info "Cloning the Redfish Mockup Server Repository"
...
This repository is a fork of the upstream DMTF project and is used here for 
documentation and monitoring examples.
...
!!! info "Running the Redfish Mockup Server"
...
You can verify that the simulator is running by querying the Redfish service root:

!!! info "Querying the Redfish Service Root"
...
!!! example "Example JSON response from the Redfish Service Root"
...
In the mock environment, the full URL is: http://127.0.0.1:8000/redfish/v1
...
![ch04.50_create_redfish_host.png](ch04.77-create_redfish_host.png)

_4.77 create Redfish host_
...
![ch04.51_create_dependent_item.png](ch04.78-create_dependent_item.png)

_4.78 Dependent item_

Let's go to our latest data page and have a look at our newly created item.

![ch04.52_latest_data.png](ch04.79-latest_data.png)
...
???+ note
    ...
    this for now and cover the [LLD chapter first](../ch08-zabbix-lld/chapter.md).)
...
![ch04.53_discovery_rule.png](ch04.80-discovery_rule.png)

_4.80 LLD Discovery rule_

The JSONPath will output each member object individually, for example:

???+ example "Redfish JSONPath output"
...
Now that we have our discovery rule it's time to create an item prototype that 
queries each discovered chassis.
...
![ch04.54_item_prototype_raw.png](ch04.81-item_prototype_raw.png)

_4.81 Item prototype raw_
...
???+example "Redfish chassis status object"
...
![ch04.55_item_prototype_health.png](ch04.82-item_prototype_health.png)

_4.82 Dependent discovery item health_
...
![ch04.56_trigger_prototype.png](ch04.83-trigger_prototype.png)

_4.83 Trigger Prototype_
