...
Все мы знаем, что прежде чем приступить к настройке Zabbix через WebUI,
...
- Требовать чтобы пароль содержал прописную и строчную латинские буквы, цифру и
  специальный символ
- Требование избегать легко угадываемых паролей
...
![Начальные настройки MFA](ch02.57-mfa_settings_initial.png){ width=90% }

_2.57 Начальные настройки MFA_

Теперь в `настройках MFA` установите флажок `Включить многофакторную
аутентификацию`, а затем в раскрывающемся списке Type (Тип) выберите `TOTP`.

![Настройки MFA TOTP](ch02.58-mfa_settings_TOTP_new.png){ width=90% }
