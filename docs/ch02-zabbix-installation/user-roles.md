...
control application functionality (what buttons a user can click).
...
that is allowed to see and modify **Templates**, but deny access to the **Hosts**
configuration to prevent accidental host changes.
...
This is perhaps the most critical section, defining **operational capabilities**.
These permissions control what actions a user can perform on existing data, regardless
of which menu they are in.
...
- **To Acknowledge Events and Add Comments:** To acknowledge (events) and provide
  ...
  not alter the core status or configuration of the problem.
...
Create a custom role for a technical administrator who manages templates but should
not modify host configurations or system settings.
...
5. **API permissions (examples)**
    ...
    * **Deny:** Host/system/user-management calls (e.g., `host.create`, `user.update`, etc.)
...
Ideal in environments with **segregated duties**, e.g., one team maintains templates
while another handles discovery and host setup.
...
!!! info "Info"
    Zabbix's User Roles and **User Groups** work together to enforce both
    **security** and **usability**. Roles define **what you can do**, while Groups
    define **what you can see** — giving administrators fine-grained control over
    both the *interface* and *data scope* in multi-team monitoring environments.
...
Zabbix's **User Roles** and **User Groups** together form the foundation of a secure
and flexible access control model. User Roles define **what a person can do**
in the frontend and API shaping the available menus, actions, and privileges.
While User Groups define **what data that person can see**, such as hosts, templates,
and triggers.

By carefully combining these two layers, administrators can tailor Zabbix to match
...
for every role within the monitoring ecosystem.
